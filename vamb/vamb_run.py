...
def calc_rpkm(jgipath, mincontiglength, refhash, ncontigs, logfile):
    begintime = time.time()
...
def trainvae(
    ...
    vae = vamb.encode.VAE(
        nsamples, nhiddens=nhiddens, nlatent=nlatent, alpha=alpha, beta=beta, dropout=dropout, cuda=cuda
    )

    log("Created VAE", logfile, 1)
    dataloader, mask = vamb.encode.make_dataloader(rpkms, tnfs, batchsize, destroy=True, cuda=cuda)
    ...
    vae.trainmodel(
        dataloader, nepochs=nepochs, lrate=lrate, batchsteps=batchsteps, logfile=logfile, modelfile=modelpath
    )
...
def cluster(
    ...
    log("Separator: {}".format(None if separator is None else ('"' + separator + '"')), logfile, 1)
    ...
    with open(clusterspath, "w") as clustersfile:
        _ = vamb.vambtools.write_clusters(
            clustersfile, renamed, max_clusters=maxclusters, min_size=minclustersize, rename=False
        )
...
def run(
    ...
    rpkms = calc_rpkm(jgipath, mincontiglength, refhash, len(tnfs), logfile)
    ...
    if minfasta is not None:
        write_fasta(outdir, clusterspath, fastapath, contignames, contiglengths, minfasta, logfile)
...
def main():
    ...
    inputos.add_argument(
        "-m", dest="minlength", metavar="", type=int, default=100, help="ignore contigs shorter than this [100]"
    )
    inputos.add_argument(
        "-s",
        dest="minascore",
    ...
    inputos.add_argument(
        ...
        help=("number of subprocesses to spawn " "[min(" + str(DEFAULT_PROCESSES) + ", nbamfiles)]"),
    )
    inputos.add_argument("--norefcheck", help="skip reference name hashing check [False]", action="store_true")
    inputos.add_argument(
        "--minfasta",
        dest="minfasta",
        metavar="",
    ...
    vaeos.add_argument(
        "-n", dest="nhiddens", metavar="", type=int, nargs="+", default=None, help="hidden neurons [Auto]"
    )
    vaeos.add_argument("-l", dest="nlatent", metavar="", type=int, default=32, help="latent neurons [32]")
    vaeos.add_argument(
        "-a", dest="alpha", metavar="", type=float, default=None, help="alpha, weight of TNF versus depth loss [Auto]"
    )
    vaeos.add_argument(
        "-b", dest="beta", metavar="", type=float, default=200.0, help="beta, capacity to learn [200.0]"
    )
    vaeos.add_argument("-d", dest="dropout", metavar="", type=float, default=None, help="dropout [Auto]")
    ...
    trainos.add_argument("-e", dest="nepochs", metavar="", type=int, default=500, help="epochs [500]")
    trainos.add_argument("-t", dest="batchsize", metavar="", type=int, default=256, help="starting batch size [256]")
    trainos.add_argument(
        ...
        type=int,
        nargs="*",
        default=[25, 75, 150, 300],
        help="double batch size at epochs [25 75 150 300]",
    )
    trainos.add_argument("-r", dest="lrate", metavar="", type=float, default=1e-3, help="learning rate [0.001]")

    # Clustering arguments
    clusto = parser.add_argument_group(title="Clustering options", description=None)
    clusto.add_argument(
        "-w", dest="windowsize", metavar="", type=int, default=200, help="size of window to count successes [200]"
    )
    clusto.add_argument(
        "-u", dest="minsuccesses", metavar="", type=int, default=20, help="minimum success in window [20]"
    )
    clusto.add_argument("-i", dest="minsize", metavar="", type=int, default=1, help="minimum cluster size [1]")
    clusto.add_argument(
        "-c", dest="maxclusters", metavar="", type=int, default=None, help="stop after c clusters [None = infinite]"
    )
    clusto.add_argument(
        "-o", dest="separator", metavar="", type=str, default=None, help="binsplit separator [None = no split]"
    )
    ...
    if args.minfasta is not None and args.fasta is None:
        raise argparse.ArgumentTypeError("If minfasta is not None, " "input fasta file must be given explicitly")
